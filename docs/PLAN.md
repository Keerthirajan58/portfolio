...
**All 7 phases below are complete.** Site is live — see the "Deployment" section of `CLAUDE.md` for the URL and operational details.
...
- Keep the old deployment live until the new site is verified and any DNS switch is confirmed. (Corrected during Phase 1: the legacy site is actually served via **GitHub Pages** from `main`, not Netlify as originally assumed here — see `CLAUDE.md`'s Deployment section.)
